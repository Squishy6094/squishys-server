...
function LGPToggle(msg)
    ...
    elseif msg == string.lower('Off') or msg == '0' then
        ...
        djui_chat_message_create("Lava Groundpound is off")
        print("Saving configuration to 'squishy's-server.sav'")
        return true
    else 
...
function AntiQS(msg)
    if msg == string.lower('On') or msg == '1' then
        if gGlobalSyncTable.GlobalAQS == false then
            djui_chat_message_create("Anti-Quicksand is forced off")
            print("Configuration save failed, settings unchanged.")
        else
            gPlayerSyncTable[m.playerIndex].AQS = true
            mod_storage_save("AQSSave", "true")
            djui_chat_message_create("Anti-Quicksand is on")
            print("Saving configuration to 'squishy's-server.sav'")
        end
        return true
    elseif msg == string.lower('Off') or msg == '0' then
        ...
        djui_chat_message_create("Anti-Quicksand is off")
        print("Saving configuration to 'squishy's-server.sav'")
        return true
    else 
        djui_chat_message_create("Please type a valid option [on|off]")
...
function GlobalAQS(msg)
    if msg == string.lower('On') or msg == '1' then
        djui_chat_message_create("Anti-Quicksand is on and can be toggled")
        gGlobalSyncTable.GlobalAQS = true
    return true
    elseif msg == string.lower('Off') or msg == '0' then
        djui_chat_message_create("Anti-Quicksand is forced off for everyone")
        gGlobalSyncTable.GlobalAQS = false
        return true
    end
end

function WKToggle(msg)
    if msg == string.lower('On') or msg == '1' then
        gPlayerSyncTable[0].wallSlide = true
        djui_chat_message_create("Modded Wallkicks are now on")
        print("Saving configuration to 'squishy's-server.sav'")
    ...
    elseif msg == string.lower('Off') or msg == '0' then
        ...
        djui_chat_message_create("Modded Wallkicks are now off")
        print("Saving configuration to 'squishy's-server.sav'")
        return true
    else 
        djui_chat_message_create("Please type a valid option [on|off]")
...
function HudToggle(msg)
	if msg == string.lower('Radar-On') or msg == '1-1' then
		gPlayerSyncTable[m.playerIndex].radarToggle = true
        mod_storage_save("CRSave", "true")
        djui_chat_message_create("Collectables Radar is now on")
        print("Saving configuration to 'squishy's-server.sav'")
		return true
	elseif msg == string.lower('Radar-Off') or msg == '1-0' then
		gPlayerSyncTable[m.playerIndex].radarToggle = false
        mod_storage_save("CRSave", "false")
        djui_chat_message_create("Collectables Radar is now off")
        print("Saving configuration to 'squishy's-server.sav'")
		return true
    elseif msg == string.lower('Cap-On') or msg == '2-1' then
        gPlayerSyncTable[m.playerIndex].capTimerToggle = true
        mod_storage_save("CTSave", "true")
        djui_chat_message_create("Cap Timer is now on")
        print("Saving configuration to 'squishy's-server.sav'")
        return true
    elseif msg == string.lower('Cap-Off') or msg == '2-0' then
        gPlayerSyncTable[m.playerIndex].capTimerToggle = false
        mod_storage_save("CTSave", "false")
        djui_chat_message_create("Cap Timer is now off")
        print("Saving configuration to 'squishy's-server.sav'")
        return true
    elseif msg == string.lower('On') or msg == '1' then
        gPlayerSyncTable[m.playerIndex].radarToggle = true
        gPlayerSyncTable[m.playerIndex].capTimerToggle = true
        mod_storage_save("CRSave", "true")
        mod_storage_save("CTSave", "true")
        djui_chat_message_create("Collectables Radar and Cap Timer is now on")
        print("Saving configuration to 'squishy's-server.sav'")
        return true
    elseif msg == string.lower('Off') or msg == '0' then
        gPlayerSyncTable[m.playerIndex].radarToggle = false
        ...
        mod_storage_save("CRSave", "false")
        mod_storage_save("CTSave", "false")
        djui_chat_message_create("Collectables Radar and Cap Timer is now off")
        print("Saving configuration to 'squishy's-server.sav'")
        return true
    else
        djui_chat_message_create("Please type a valid option [on|off]")
...
function on_exclude_levels_command(msg)
    if msg == string.lower('On') or msg == '1' then
        gGlobalSyncTable.excludeLevels = true
        djui_chat_message_create("Unstable Door Bust Levels Excluded")
    elseif msg == string.lower('Off') or msg == '0' then
        gGlobalSyncTable.excludeLevels = false
        djui_chat_message_create("Unstable Door Bust Levels Included")
    else
        djui_chat_message_create("Please type a valid option [on|off]")
        return true
    end
...
if gPlayerSyncTable[0].CMDToggle == true then
    hook_chat_command("modwallkick", "[on|off] - Toggles the Custom Wallkick.", WKToggle)
    hook_chat_command("afk", "[on|off]", AFK)
    hook_chat_command('extra-hud', '[radar|cap]-[on|off] - Displays distance to nearest red coin or secret and a timer for your wing cap duration.', HudToggle)
    hook_chat_command('lavagp', '[on|off] - Toggles groundpounding lava giving you a boost at the cost of health.', LGPToggle)
    hook_chat_command('antiqs', '[on|off] - Toggles Instant Quicksand burning you like lava.', AntiQS)
end

if network_is_server() or network_is_moderator() then
    hook_chat_command("canafk", "[on|off] Allow/Disallow players afking", CANAFK)
    hook_chat_command("globalaqs", "[on|off] Change if Quicksand acts like lava globally, If 'on' then players can locally choose", GlobalAQS)
