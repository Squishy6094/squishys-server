...
if _G.ssExists then
    inGame = false
    _G.ssApi.option_add("Play Luigi's Casino", 0, 1, {[0] = "", [1] = ""}, {
        "Introducing Luigi's Casino!",
        "Hosted by Luigi himself, and",
        "featuring the nostalgic",
        "Picture Poker minigame from",
        "SM64DS and NSMB!",
        "",
        "",
        "",
...
function notifynewplayers()
    if _G.ssExists then
        djui_chat_message_create("This server has \\#007700\\Luigi's Casino\\#ffffff\\ enabled! You can play the game by using the Squishy's Server Menu (External Tab)!")
    end
end

function check_status_change()
    if not _G.ssBooted then return end
    if _G.ssApi.option_read("Play Luigi's Casino") == 1 then
        inGame = true
    end
end
...
local function on_hud_render()
    ...
    if show_info then
        ...
        if _G.ssExists then
            djui_hud_print_text("You can quit the game by typing '/quit'.", (djui_hud_get_screen_width()/3)*2.10, djui_hud_get_screen_height() - 65, 0.25)
        end
...
local function server_update()
    if inGame then
        ...
        for i = 0, MAX_PLAYERS - 1 do
            local m = gMarioStates[i]
            if not gNetworkPlayers[m.playerIndex].connected then return end
            network_player_set_description(gNetworkPlayers[m.playerIndex], "", 255, 255, 255, 255)
        end
    end
end
...
local function quit(msg)
    if _G.ssExists then
        ...
        _G.ssApi.option_write("Play Luigi's Casino", 0)
        inGame = false
...
local function on_chat_message(m, msg)
    if _G.ssExists then return false end
    if inGame then
...
_G.lcApi.get_ranks = function()
    return tostring(titles[player_title1 + 1].." "..titles[player_title2 + 1])
end
...
hook_chat_command("quit", "- \\#00ff00\\[Luigi's Casino]\\#ffffff\\ Quit the game if you're using Squishy's Server.", quit)
